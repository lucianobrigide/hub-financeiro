...
- **Amazon Ads (30/07/2026 — Fase 1 pronta, autorização pendente)** — app LwA separada da SP-API, prefixo **`azads_`** (migration `20260730120001_azads_oauth`). Custódia no Vault (`azads_client_id/_secret/_refresh_token/_token_key`); troca do code **dentro do Postgres** (`azads_exchange_code`, padrão TikTok) via rota `app/api/auth/callback-amazon-ads`; gateway Edge `amazon-ads-token` (espelho da `az-token`, testado 30/07). **03/08:** Vercel pausada por estouro de cota free (3M/1M edge requests, só religa pagando) → callback migrado para Edge **`amazon-ads-callback`** (padrão tt, testado ponta a ponta; `azads_exchange_code` com `p_redirect_uri` + allowlist, migration `20260803120001`); falta o Luciano cadastrar a Return URL nova no security profile LwA e autorizar. Roteiro completo + descoberta (item 5/6) em `AMAZON_ADS_DESCOBERTA.md`. **Sem cron, sem DRE, card intocado** até o item 6 bater.
...
- 🔺 **Amazon ADS = R$0 HARDCODED** (`lib/data/supabase.ts`) — junho ok (não anunciou), **julho ERRADO**: o canal explodiu de 7 para 80 pedidos justo quando o ADS foi ligado; a 12–15% de ADS a margem zera/inverte. Exige a **Advertising API** separada (a do SP-API não serve). **EM ANDAMENTO (30/07/2026):** Fase 1 (custódia `azads_`, callback, Edge `amazon-ads-token`) pronta e testada; falta autorizar (bloqueado pela Vercel pausada) e validar item 6 antes de mexer no card — ver `AMAZON_ADS_DESCOBERTA.md` e §Integrações/Amazon Ads.
