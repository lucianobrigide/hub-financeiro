-- Agenda os crons do Amazon Ads e atualiza o catálogo do /crons.
-- Gate cumprido em 03/08/2026: item 6 bateu EXATO (API R$ 14.125,85 = painel
-- R$ 14.125,85, 0,000%; cliques 18.927 = 18.927) antes de qualquer agendamento.
-- Horários UTC (BRT = -3): diário 06:45 (03:45 BRT, depois do az-diario 03:15 e
-- ml-ads-item 03:25); colheita extra 07:25 (04:25 BRT) pega relatório que a
-- Amazon demora a gerar (medido: 10-15 min no backfill de julho).
-- azads-colher fica FORA do catálogo do /crons, como os auxiliares shopee-escrow-*.

select cron.schedule('azads-diario', '45 6 * * *',
  $$SET statement_timeout='5min'; SELECT public.azads_cron_diario()$$);
select cron.schedule('azads-colher', '25 7 * * *',
  $$SET statement_timeout='5min'; SELECT public.azads_cron_colher()$$);

-- Catálogo do /crons: entra azads-diario; az-diario perde o "(ADS ainda não é
-- capturado.)". Base: definição viva capturada via pg_get_functiondef 03/08/2026.
