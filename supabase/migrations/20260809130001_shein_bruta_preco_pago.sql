-- SHEIN: bruta = preço PAGO pelo cliente (decisão do Luciano, 09/08/2026).
--
-- Contexto: productTotalPrice (régua anterior) é o preço CHEIO de tabela
-- (sellerCurrencyPrice) — ago/2026 dava R$ 53.081,90 contra GMV R$ 25.430,58
-- na central de vendedores (01-08/08), e deixava o CMV% em 36% (base inflada).
--
-- Régua nova (tudo REAL do order-detail, por ITEM — validado que Σ itens =
-- pedido centavo a centavo em jul e ago/2026):
--   bruta            = Σ sellerCurrencyDiscountPrice (preco_com_desconto)
--   Comissão / Taxa  = reais do item (batem com o total do pedido)
--   Subsídio SHEIN   = (preco − cupom − promo) − preco_com_desconto, entra como
--                      CRÉDITO nas deduções (a SHEIN calcula o repasse sobre
--                      base MAIOR que o preço pago); ago/2026: R$ 7.685,77
--   Cupons/promos da loja deixam de ser linha — já embutidos no preço pago.
-- Identidade preservada: pago − (comissão + taxa − subsídio) = estimatedGrossIncome
-- ⇒ a M.C. em reais NÃO muda (ago/2026: R$ 3.430,75).
--
-- Divergência residual conhecida vs central (01-08/08: 178 ped/R$ 23.206,65
-- aqui contra 185 ped/GMV 25.430,58 lá): pedidos reembolsados que o
-- order-detail ZERA ou o order-list omite — a central conta a venda original.
-- Re-sync manual 09/08 confirmou que a API não devolve esses pedidos.
-- Conciliação exata fica para a fase do settlement. Nada estimado (REGRA DURA).
--
-- Mesma régua na série diária (dre_diario_canais) e no dre_sku.

-- ── RPCs do card: bruta = preço PAGO pelo cliente ──
...
as $$
  with base as (
    select p.order_no, p.order_status, coalesce(sum(i.preco_com_desconto), 0) pago
    from public.shein_pedidos p
    left join public.shein_itens i on i.order_no = p.order_no
    where to_char(coalesce(p.payment_time, p.order_time) at time zone 'America/Sao_Paulo', 'YYYY-MM') = p_month
    group by 1, 2
  )
  -- bruta ⊃ devoluções (semântica do card ML): líquido = bruta − devoluções
  -- = pedidos válidos, e a identidade com o estimatedGrossIncome fecha exata.
  select jsonb_build_object(
    'faturamento_bruto', coalesce(sum(pago), 0),
    'cancel_devolucoes', coalesce(sum(pago) filter (where order_status in (6,8,9)), 0),
    'total_pedidos',     count(*) filter (where order_status not in (6,8,9))
  ) from base;
$$;

drop function if exists public.shein_deducoes(text);
create or replace function public.shein_deducoes(p_month text)
returns jsonb language sql stable security definer
set search_path to 'public'
as $$
  select jsonb_build_object(
    'comissao',     coalesce(sum(i.comissao), 0),
    'taxa_servico', coalesce(sum(i.taxa_servico), 0),
    'subsidio',     coalesce(sum((coalesce(i.preco,0)-coalesce(i.cupom,0)-coalesce(i.promo,0)) - coalesce(i.preco_com_desconto,0)), 0)
  )
...
as $function$
  ...
  sh as (
    select i.seller_sku sku, max(i.goods_title) titulo,
      ...
      sum(coalesce(i.comissao,0)+coalesce(i.taxa_servico,0)
          - ((coalesce(i.preco,0)-coalesce(i.cupom,0)-coalesce(i.promo,0)) - coalesce(i.preco_com_desconto,0))) comissao,
      0::numeric frete, 0::numeric ads
    from public.shein_itens i
    join public.shein_pedidos p on p.order_no=i.order_no
