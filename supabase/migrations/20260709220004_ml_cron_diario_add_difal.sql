-- Adiciona o passo 'difal' (CDIFAL) ao orquestrador diário do ML, logo após afiliados.
-- Mesmo padrão: chama a Edge modo='difal' (ingestDifal) e loga 'diario_difal'. Assim a
-- linha DIFAL (7ª dedução) do DRE ML se atualiza no fluxo noturno (ml-diario 03:00 BRT),
-- pescando ciclo atual + anterior por creation_date.
...
begin
  ...
  -- NOVO: DIFAL (CDIFAL) — pesca ciclo atual + anterior, régua creation_date.
  v_t := clock_timestamp();
