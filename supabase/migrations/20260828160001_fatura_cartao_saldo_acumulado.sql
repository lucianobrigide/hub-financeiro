-- ═══════════════════════════════════════════════════════════════════════════
-- Fatura de cartão: o SALDO JÁ LANÇADO no cartão entra na projeção
-- (28/08/2026, sequência da 20260828150001; pedido do Luciano: "considerar o
-- total do cartão de crédito — despesa programada OU já realizada no sistema")
--
-- Furo achado na verificação: quando a equipe BAIXA um título contra a conta
-- do cartão (tipo CR), ele vira PAGO na AP e sumia da projeção — mas o caixa
-- só sai quando a FATURA é paga. O débito acumulado fica no saldo da conta CR
-- (`omie_saldos_cc.saldo_atual` negativo; coletado 2×/dia pelo omie-saldos).
-- Medido em 28/08: Visa 2439 −R$ 10.915,41 e MasterCard −R$ 2.321,60 já
-- lançados e SEM projeção; a fatura real de 10-11/09 é saldo + abertos.
--
-- Régua: para cada cartão com saldo_atual < 0, entra um item sintético
-- "Fatura <cartão>" na PRÓXIMA data de fatura (mesma régua da 150001: dia do
-- último pagamento visto no extrato, fallback 10), valor = |saldo_atual|.
-- Sem dupla contagem: cada despesa de cartão está OU aberta na AP (projetada
-- individualmente na fatura) OU baixada (dentro do saldo do cartão) — quando a
-- Fernanda baixa, ela migra de um balde para o outro e o total se mantém.
-- Quando a fatura é paga, a transferência banco→cartão zera o saldo e o item
-- some sozinho.
-- ═══════════════════════════════════════════════════════════════════════════
...
'Saídas projetadas do F.C. (contas a pagar Omie, previsão de pagamento, corte fixo 01/08/2026). Cartão de crédito (28/08/2026): título de conta CR é projetado na PRÓXIMA data de fatura do cartão (dia = último pagamento de fatura no extrato, fallback 10, auto-ajusta), nunca vencido; e o SALDO JÁ LANÇADO no cartão (títulos baixados aguardando fatura, omie_saldos_cc.saldo_atual < 0) entra como item sintético "Fatura <cartão>" na mesma data — fatura projetada = saldo + abertos, sem dupla contagem (cada despesa está num balde só).';
