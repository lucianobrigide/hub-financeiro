-- CMV com vigência temporal (2026-08-03).
-- ml_custo_produto deixa de ter um custo único por SKU: cada linha passa a valer no
-- intervalo [vigencia_inicio, vigencia_fim) — fim NULL = vigente. Linhas pré-existentes
-- recebem vigencia_inicio '2000-01-01' (default) e fim NULL, preservando todo o histórico.
--
-- Fluxo de alteração de custo: NUNCA UPDATE no valor — fechar a linha vigente
-- (vigencia_fim = data da mudança) + inserir linha nova. O trigger
-- ml_custo_produto_custo_imutavel bloqueia UPDATE de custo; use cmv_alterar_custo().
--
-- Todas as 9 RPCs que leem a tabela passam a fazer range join ancorado na DATA DE
-- COMPETÊNCIA da venda de cada canal (mesma âncora que cada RPC já usava no WHERE):
--   ML  = ml_pedidos.data (date_closed @SP)   Shopee/TikTok = create_time @SP
--   Amazon = purchase_date                    B2B = data_emissao
--
-- Rollback: scripts/cmv-regression/rollback_cmv_vigencia.sql
-- Não-regressão: scripts/cmv-regression/{snapshot,diff}.mjs (meses fechados batem exatamente).

-- 1 · Extensão para o EXCLUDE (igualdade de text em índice gist)
create extension if not exists btree_gist with schema extensions;

-- 2 · Colunas de vigência
alter table public.ml_custo_produto
  add column vigencia_inicio date not null default '2000-01-01',
  add column vigencia_fim date;

-- 3 · PK composta + garantia de não-sobreposição de vigências por SKU
alter table public.ml_custo_produto drop constraint ml_custo_produto_pkey;
alter table public.ml_custo_produto
  add constraint ml_custo_produto_pkey primary key (sku, vigencia_inicio);
alter table public.ml_custo_produto
  add constraint ml_custo_produto_vigencia_valida
  check (vigencia_fim is null or vigencia_fim > vigencia_inicio);
alter table public.ml_custo_produto
  add constraint ml_custo_produto_sem_sobreposicao
  exclude using gist (
    sku extensions.gist_text_ops with =,
    daterange(vigencia_inicio, vigencia_fim, '[)') with &&
  );

comment on column public.ml_custo_produto.vigencia_inicio is
  'Início (inclusive) da vigência do custo. Linhas originais: 2000-01-01.';
comment on column public.ml_custo_produto.vigencia_fim is
  'Fim (exclusive) da vigência. NULL = vigente. Nunca reabrir/alterar custo de linha fechada.';
comment on table public.ml_custo_produto is
  'De-para variante(SKU vendido) -> modelo -> CMV, com vigência temporal [inicio, fim). '
  'Custo por modelo vindo de PRODUTOS.xlsx. origem: confirmado (dono validou variante), '
  'exato (SKU vendido = SKU planilha), proposto-prefixo (casou por prefixo, dono validou em jun/2026). '
  'Alteração de custo: cmv_alterar_custo() — fecha a vigente e insere nova; UPDATE de custo é bloqueado por trigger.';

-- 4 · Trigger: custo é imutável (mudança = fechar vigência + inserir linha nova)
create or replace function public.ml_custo_produto_guard()
returns trigger language plpgsql as $$
begin
  if new.custo is distinct from old.custo then
    raise exception 'custo é imutável (sku %, vigência desde %): feche a vigência e insira linha nova — use cmv_alterar_custo()',
      old.sku, old.vigencia_inicio;
  end if;
  return new;
end $$;
create trigger ml_custo_produto_custo_imutavel
  before update on public.ml_custo_produto
  for each row execute function public.ml_custo_produto_guard();

-- 5 · Fluxo oficial de alteração de custo
create or replace function public.cmv_alterar_custo(
  p_sku text, p_novo_custo numeric, p_vigencia date,
  p_origem text default 'confirmado', p_modelo text default null
) returns public.ml_custo_produto
language plpgsql security definer set search_path to 'public' as $$
declare
  v_vigente public.ml_custo_produto;
  v_nova public.ml_custo_produto;
begin
  select * into v_vigente from ml_custo_produto where sku = p_sku and vigencia_fim is null;
  if found then
    if v_vigente.vigencia_inicio >= p_vigencia then
      raise exception 'vigência atual de % começa em % (>= %): ajuste inválido', p_sku, v_vigente.vigencia_inicio, p_vigencia;
    end if;
    if v_vigente.custo = p_novo_custo then
      raise exception 'custo de % já é %: nada a alterar', p_sku, p_novo_custo;
    end if;
    update ml_custo_produto set vigencia_fim = p_vigencia, atualizado_em = now()
      where sku = p_sku and vigencia_inicio = v_vigente.vigencia_inicio;
  end if;
  insert into ml_custo_produto (sku, modelo, custo, origem, vigencia_inicio)
  values (p_sku, coalesce(p_modelo, v_vigente.modelo, p_sku), p_novo_custo, p_origem, p_vigencia)
  returning * into v_nova;
  return v_nova;
end $$;
revoke all on function public.cmv_alterar_custo(text, numeric, date, text, text) from public, anon, authenticated;

-- 6 · RPCs: range join na vigência, ancorado na competência de cada canal ------------
...
as $function$
  ...
  cmv as (
    ...
    join public.ml_custo_produto c on c.sku=i.sku
      and b.data >= c.vigencia_inicio and (c.vigencia_fim is null or b.data < c.vigencia_fim)
    group by b.data),
  com as (
