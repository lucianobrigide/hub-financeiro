-- crons_status: entra a SHEIN no catálogo (shein-diario / shein-semanal).
-- Também soma as falhas de fase do shein_fill (ml_cron_log, job like 'shein%')
-- no falhas_24h — mesmo mecanismo da 20260805150002. Restante inalterado.
...
AS $function$
  with catalogo(jobname, plataforma, categoria, horario, o_que_faz, confiab_log, atraso_ok_horas, log_conta) as (values
    ...
    ('shein-diario','SHEIN','diario','Todo dia às 03:50 BRT',
     'Puxa os pedidos dos últimos 3 dias da SHEIN (order-list + order-detail, janelas de 48h) com comissão, taxa de serviço e cupons reais de cada pedido. É o que mantém o card SHEIN em dia.',
     'honesto', 28, null),
    ...
    ('az-semanal','Amazon','semanal','Todo domingo às 05:30 BRT',
     ...
     'honesto', 192, null),
    ('shein-semanal','SHEIN','semanal','Todo domingo às 05:35 BRT',
     'Reconferência longa: revisa 30 dias da SHEIN pra pegar devoluções, recusas e mudanças de status que o diário (3 dias) não alcança.',
     'honesto', 192, null),
  ...
  -- falhas nas últimas 24h: pg_cron (exceção crua) + logs de aplicação (falha de fase)
  falhas24 as (
    select jobname, count(*) as falhas_24h,
           max(em) as ultima_falha_em,
           (array_agg(msg order by em desc))[1] as ultima_falha_msg
    from (
      select j.jobname, d.start_time as em,
             left(regexp_replace(d.return_message, '\s+', ' ', 'g'), 200) as msg
      from cron.job j
      join cron.job_run_details d on d.jobid = j.jobid
      where d.status = 'failed' and d.start_time >= now() - interval '24 hours'
      union all
      select case o.conta when 'shopee_cron' then 'shopee-diario'
                          when 'tt_cron'     then 'tt-diario' end,
             o.created_at,
             left(regexp_replace(o.message, '\s+', ' ', 'g'), 200)
      from oauth_refresh_log o
      where o.success is false and o.created_at >= now() - interval '24 hours'
        and o.conta in ('shopee_cron','tt_cron')
      union all
      select case when m.job like 'diario%' then 'ml-diario'
                  when m.job like 'az-%'    then 'az-diario'
                  when m.job like 'azads%'  then 'azads-diario'
                  when m.job like 'shein%'  then 'shein-diario' end,
             m.executado_em,
             left(regexp_replace(m.job || ': ' || coalesce(m.mensagem, 'falhou'), '\s+', ' ', 'g'), 200)
      from ml_cron_log m
      where m.sucesso is false and m.executado_em >= now() - interval '24 hours'
        and (m.job like 'diario%' or m.job like 'az-%' or m.job like 'azads%' or m.job like 'shein%')
    ) f
