-- dre_diario_canais: separa o SUBSÍDIO da SHEIN da linha de comissão.
--
-- Antes, o ramo 'sh' devolvia comissao = (comissão + taxa de serviço − subsídio), ou seja,
-- uma dedução LÍQUIDA. A M.C. fechava certo, mas a composição era ilegível: o gráfico
-- mostrava ~8% de take rate quando o real é ~27%, porque o crédito do subsídio estava
-- escondido dentro da comissão.
--
-- Agora:
...
--
-- Régua do subsídio (idêntica à do card, shein_deducoes): a SHEIN liquida sobre
-- (preço de tabela − promo), não sobre o que o cliente pagou. A diferença
-- ((preco − cupom − promo) − preco_com_desconto) é dinheiro que ela repassa.
--
-- M.C. do dia passa a ser: fat − cmv − comissao − frete − ads + subsidio.
-- Demais canais devolvem subsidio = 0 (o campo existe para todos, união uniforme).
-- Nenhum valor de M.C. muda — só a decomposição.
...
AS $function$
  ...
  sh_ped as (
    select (coalesce(p.payment_time, p.order_time) at time zone 'America/Sao_Paulo')::date d,
           ...
           sum(coalesce(i.comissao,0)+coalesce(i.taxa_servico,0)) ded_bruta,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'frete',round(frete,2),'ads',round(ads,2),'subsidio',round(subsidio,2)
