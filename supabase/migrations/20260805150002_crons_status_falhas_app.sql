-- crons_status: falhas de FASE também contam (complemento da 20260805140002).
--
-- Com as fases resilientes (20260805140001 Shopee, 20260805150001 ML/TT/AZ),
-- uma falha de fase não derruba mais a função — o run do pg_cron aparece como
-- "succeeded" e a falha fica só no log de aplicação. Este ajuste faz o
-- `falhas_24h` somar TAMBÉM os registros de falha dos logs de aplicação:
--   - oauth_refresh_log success=false: shopee_cron → shopee-diario; tt_cron → tt-diario
--   - ml_cron_log sucesso=false: diario_% → ml-diario; az-% → az-diario; azads% → azads-diario
-- Assim o semáforo amarelo ("rodou, mas teve falha nas 24h") continua funcionando
-- mesmo sem o pg_cron nunca mais ver a exceção. Restante da função inalterado.
  ...
  falhas24 as (
    ...
    from (
      ...
      select case when m.job like 'diario%' then 'ml-diario'
                  ...
                  when m.job like 'azads%'  then 'azads-diario' end,
             m.executado_em,
      ...
      where m.sucesso is false and m.executado_em >= now() - interval '24 hours'
        and (m.job like 'diario%' or m.job like 'az-%' or m.job like 'azads%')
    ) f
