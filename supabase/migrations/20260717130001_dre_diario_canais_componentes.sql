-- Estende dre_diario_canais: além do faturamento, devolve os custos diretos SEPARADOS
-- por dia (cmv, comissao, frete, ads) para o gráfico de despesas detalhadas por canal.
-- (O resíduo mensal — afiliados/DIFAL/devoluções — vira "Outras" rateado na camada TS.)
-- Componentes validados vs cards (jun): Shopee CMV 648.880,65 · comissão+afiliados 248.811,76 ·
-- ADS 48.368,82; Amazon comissão 279,36 · frete 251,55; TikTok comissão+taxas 3.706,15 · frete 500,85.
...
AS $function$
  ...
  sp as (
    select 'sp'::text canal, sp_ped.d, sp_ped.fat,
      coalesce(sp_cmv.cmv,0) cmv, (sp_ped.fat - sp_ped.repasse) comissao, 0::numeric frete,
      coalesce(sp_ads.ads,0) ads
    from sp_ped left join sp_cmv on sp_cmv.d=sp_ped.d left join sp_ads on sp_ads.d=sp_ped.d),
  tt_ped as (
    select (create_time at time zone 'America/Sao_Paulo')::date d,
           sum(fin_revenue) fat, -sum(coalesce(fin_fee_tax,0)) taxas, -sum(coalesce(fin_frete,0)) frete
