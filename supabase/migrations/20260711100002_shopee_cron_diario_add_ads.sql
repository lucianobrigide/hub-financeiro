-- Adiciona FASE 4 (ADS) ao orquestrador diário da Shopee. Pesca o gasto CPC diário de
-- 2 janelas (mês atual MTD + mês anterior completo — a API limita o range a 1 mês).
-- NÃO agenda nada novo: o job shopee-diario (03:30 BRT) já existe; só o passo foi somado.
...
DECLARE
  ...
  v_today date;
  v_ads_cur jsonb; v_ads_prev jsonb;
  v_ads_dias int := 0; v_ads_expense numeric := 0;
BEGIN
  ...
  v_ads_expense := COALESCE((v_ads_cur->>'expense_total')::numeric, 0)
                 + COALESCE((v_ads_prev->>'expense_total')::numeric, 0);
  ...
  RETURN jsonb_build_object(
    ...
    'escrow_pending', v_escrow_remaining,
    'ads_dias',    v_ads_dias,
    'ads_expense', round(v_ads_expense, 2)
  );
END
$$;
