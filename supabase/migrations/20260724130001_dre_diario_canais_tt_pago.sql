-- Gráfico "Faturamento e M.C. por dia" (RPC dre_diario_canais): as CTEs do TikTok (tt_ped/tt_cmv)
-- ainda usavam a régua antiga (fin_revenue + fin_filled), então o gráfico do TikTok não mostrava
-- os dias com pedido pago-não-liquidado. Alinha ao card (migration 20260724120001) e ao Shopee,
-- que ali já conta antes de liquidar. (Luciano, 24/07/2026.)
--   tt_ped.fat  = fin_revenue quando liquidado, senão payment_total; exclui UNPAID/CANCELLED.
--   tt_cmv      = custo do produto de todo pedido pago (sem fin_filled).
--   taxas/frete diários seguem só-liquidado (deduções vêm na liquidação).
-- Só as CTEs tt_ped e tt_cmv mudaram; o resto do RPC é idêntico.
...
AS $function$
  ...
  az as (
    select 'az'::text canal, az_ped.d, az_ped.fat,
      coalesce(az_cmv.cmv,0) cmv, coalesce(az_com.com,0) comissao, coalesce(az_fr.fr,0) frete, 0::numeric ads
