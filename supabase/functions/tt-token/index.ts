...
// Porta de LEITURA do token OAuth TikTok Shop (get-or-refresh) + assinatura HMAC.
// 1) lê tt_oauth_state; 2) cache válido (folga 7 dias) devolve sem rotacionar;
// 3) senão delega ao RPC tt_refresh_token e relê.
// Se query param ?path= fornecido, retorna params assinados (HMAC) para a chamada.
// Auth por x-api-key validada via tt_token_check.

const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const SKEW_MS = 1 * 24 * 60 * 60 * 1000;
...
type State = { access_token: string | null; shop_cipher: string | null; expires_at: string | null };

async function readState(): Promise<State | null> {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/tt_get_state`, {
...
async function keyIsValid(candidate: string | null): Promise<boolean> {
  ...
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/tt_token_check`, {
...
async function getSignedParams(path: string): Promise<Record<string, unknown> | null> {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/tt_signed_params`, {
...
Deno.serve(async (req) => {
  ...
  if (!isFresh(state)) {
    const rpc = await fetch(`${SUPABASE_URL}/rest/v1/rpc/tt_refresh_token`, {
    ...
    if (result && result.valid === false) {
      return json({
        ...
        hint: result.error === "refresh_token_not_seeded"
          ? "Autorize o app TikTok Shop primeiro via tt_auth_url()."
          : "Verifique vault.secrets (tt_*) e public.oauth_refresh_log.",
      }, 502);
    }
